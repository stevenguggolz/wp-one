...
class OD_Image extends Widget_Base {
     ...
     * @param null $image_key
     *
     * @return string Image URL.
     */
    public static function get_attachment_image_url(  $settings, $image_size_key = 'image', $image_key = null) {
        if ( ! $image_key ) {
	...
	protected function render() {
		...
		if ( empty( $settings['image']['url'] ) && empty( $settings['image_tablet']['url'] ) && empty( $settings['image_mobile']['url'] )) {
			return;
		}
			...
			<?php endif; ?>
                <picture>
                    <?php if($settings['image']['url']):
                        $imageSizeKey = 'image';
                        ?>
                        <source media="(min-width: 1200px)" srcset="<?php echo self::get_attachment_image_url($settings, 'image'); ?>">
                    <?php endif; ?>
                    <?php if($settings['image_tablet']['url']):
                        if($imageSizeKey === ''){
                            $imageSizeKey = 'image_tablet';
                        }
                        ?>
                        <source media="(min-width: 768px)" srcset="<?php echo self::get_attachment_image_url($settings, 'image_tablet'); ?>">
                    <?php endif; ?>
                    <?php if($settings['image_mobile']['url']):
                        if($imageSizeKey === ''){
                            $imageSizeKey = 'image_mobile';
                        }?>
                        <source srcset="<?php echo self::get_attachment_image_url($settings, 'image_mobile'); ?>">
                    <?php endif; ?>

				    <?php Group_Control_Image_Size::print_attachment_image_html( $settings, $imageSizeKey); ?>
                </picture>
			<?php if ( $link ) : ?>
					</a>
			<?php endif; ?>
